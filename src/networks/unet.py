...
from networks.feature_perturbe import FeaturePerturbation
...
class Decoder_NP(nn.Module):
    def __init__(self, params):
        ...
        self.out_conv = nn.Conv2d(self.ft_chns[0], self.n_class,
                                  kernel_size=3, padding=1)
        self.cont_conv_a = nn.Conv2d(
            self.ft_chns[0], self.ft_chns[0], kernel_size=1, padding=0)
        self.cont_conv_b = nn.Conv2d(
            self.ft_chns[0], self.ft_chns[0], kernel_size=1, padding=0)
...
class UNet_W2S(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_NP(params)
        self.decoder_1 = Decoder_W2S(params, kaps[0])
        self.decoder_2 = Decoder_W2S(params, kaps[1])
        self.decoder_3 = Decoder_W2S(params, kaps[2])
...
if __name__ == '__main__':
    ...
    print(outpust[0][0].size(), outpust[0][1].size(),
          outpust[1][0].size(), outpust[1][1].size())
